...
pub fn polyline_path_segs(
    vertices: &[cad_core::PolyVertex],
    closed: bool,
    extrusion: cad_core::Point3,
    transform: cad_core::Transform2,
) -> Vec<PathSeg> {
...
fn poly_vertex_seg(
    a: cad_core::PolyVertex,
    b: cad_core::PolyVertex,
    extrusion: cad_core::Point3,
    transform: cad_core::Transform2,
) -> PathSeg {
    let to_world = |sample: Point2, elevation: f64| {
        transform.apply(
            cad_core::ocs_to_wcs(
                cad_core::Point3::new(sample.x, sample.y, elevation),
                extrusion,
            )
            .xy(),
        )
...
mod tests {
    use super::*;
    use cad_core::{Point3, PolyVertex, Transform2};
